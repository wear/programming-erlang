...
init([]) ->
  ...
  timer:send_interval(?TIME, self(), lazy_check),
  {ok, Tab}.
...
handle_call(wanted, {From, _}, Tab) ->
  case ets:match(Tab, {'$1', ?StatusPending, '$2'}) of
    [] -> {reply, no, Tab};
    [[N, F]|_T] ->
      ets:insert(Tab, [
        {N, ?StatusTaken, F},
        {From, {N, F}, now_in_secs(), working}]),
...
handle_info({'DOWN', _, process, Pid, Why}, Tab) ->
  io:format("~p down because of ~p", [Pid, Why]),
  case ets:lookup(Tab, Pid) of
    [] -> {noreply, Tab};
    [{Pid, {N, F}, _, _}] ->
      ets:insert(Tab, {N, ?StatusPending, F}),
      {noreply, Tab}
  end;
handle_info(lazy_check, Tab) ->
  case ets:match(Tab, {'$1', '_', '$2', working}) of
    [] -> {noreply, Tab};
    WorkingJobs ->
      Now = now_in_secs(),
      lists:foreach(
        fun(Job) ->
          ...
          if
            Now =:= T + ?WorkingSec - 1 ->
              From ! hurry_up;
            Now =:= T + ?WorkingSec + 1 -> exit(From, youre_fired);
            true -> ok
          end
        end,
        WorkingJobs),
